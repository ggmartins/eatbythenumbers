...
d3.csv("nutrients.csv", function(raw_data) {
  ...
  data = raw_data.map(function(d) {
    for (var k in d) {
      if (!_.isNaN(raw_data[0][k] - 0) && k != 'id' && k != 'Latitude' && k != 'Longitude') {
    ...
    .append("span")
      .text(function(d) 
        { 
          ...
          //setMarkers(map, ["test", 40.294019, -74.682432, 0]);
          //setMarkers(map, ["Name0",0,0,0]);
          setMarkers(map, [d.name, Number(d.Latitude), Number(d.Longitude), 0]);
          return d.name; 
        })
}
...
function update_ticks(d, extent) {
  ...
  if (d) {
    var brush_el = d3.selectAll(".brush")
        .filter(function(key) { return key == d; });//.on("dragstart")
    // single tick
    if (extent) {
